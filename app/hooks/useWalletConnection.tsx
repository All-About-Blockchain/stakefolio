...
export const useWalletConnection = () => {
  ...
  const updateConnectionStatus = useCallback((chainName: string, status: Partial<ConnectionStatus>) => {
  ...
  const getConnectionSummary = useCallback((): ConnectionSummary => {
    ...
    const connected = connectionStatuses.filter(s => s.status === 'connected').length;
    const connecting = connectionStatuses.filter(s => s.status === 'connecting').length;
    const failed = connectionStatuses.filter(s => s.status === 'failed').length;
    const disconnected = connectionStatuses.filter(s => s.status === 'disconnected').length;
  ...
  const connectSingleChain = useCallback(async (chainName: string): Promise<void> => {
  ...
  const disconnectSingleChain = useCallback(async (chainName: string): Promise<void> => {
  ...
  const connectAll = useCallback(async (): Promise<ConnectionResult | null> => {
    ...
    const disconnectedChains = connectionStatuses.filter(s => s.status === 'disconnected');
  ...
  const disconnectAll = useCallback(async (): Promise<void> => {
    const connectedChains = connectionStatuses.filter(s => s.status === 'connected');
  ...
  const retryFailed = useCallback(async (): Promise<ConnectionResult | null> => {
    if (isConnecting) return null;
    
    setIsConnecting(true);
    const failedChains = connectionStatuses.filter(s => s.status === 'failed');
